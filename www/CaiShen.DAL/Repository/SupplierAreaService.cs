...
{
    public class SupplierAreaService
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static SupplierArea GetModel(int ID)
        {
            var list = work.SupplierAreaRepository.Get(m => m.ID == ID, null).ToList<SupplierArea>();
            if (list.Count() > 0)
            {
                return list[0];
            }
            return new SupplierArea();
        }
        #endregion
        ...
        /// <returns></returns>
        public static bool CheckIsSameArea(int SupplierID, int userAddressID)
        {
            //地址模板
            ...
            var rst_area = work.SupplierAreaRepository.Get(m => m.SupplierID == SupplierID).ToList();
        ...
        /// <returns></returns>
        public static bool CheckIsSameArea(int SupplierID, UserShop shopModel)
        {
            var rst_area = work.SupplierAreaRepository.Get(m => m.SupplierID == SupplierID).ToList();
